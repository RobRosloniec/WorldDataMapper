...
export class SortRegions_Transaciton extends jsTPS_Transaction{
    ...
    async doTransaction() {
        const { data } = await this.callback({variables: {_id: this.regionID, criteria: this.nextSortRule}});
        if(data) {
            console.log(data)
            return data;
    ...
    async undoTransaction() {
        const { data } = await this.callback({variables: {_id: this.regionID, criteria: this.prevSortRule}});
        if(data) {
            console.log(data)
            return data;
...
export class EditRegion_Transaction extends jsTPS_Transaction {
    constructor(regionID, field, newVal, prevVal, callback) {
        super();
        this.regionID = regionID;
        this.field = field;
        this.newVal = newVal;
    ...
    async doTransaction() {
        const { data } = await this.callback({variables: {_id: this.regionID , field: this.field, value: this.newVal}});
        return data;
    }

    async undoTransaction() {
        const { data } = await this.callback({variables: {_id: this.regionID , field: this.field, value: this.prevVal}});
        return data;
    }
}
...
export class UpdateRegions_Transaction extends jsTPS_Transaction {
    constructor (regionID, region, addfunc, delfunc, index, opcode){
        super();
        this.regionID = regionID;
        this.region = region;
    ...
    async doTransaction() {
        let data;
        this.opcode === 0 ? { data } = await this.delfunc({ variables: {_id: this.regionID}})
        : { data } = await this.addfunc({variables: { region: this.region, _id: this.regionID, index: this.index}})
        if (this.opcode === 1){
            this.delRegionID = data.addRegion
        }
        return data;
    }
    ...
    async undoTransaction() {
        ...
        console.log(this.region.parentregion)
        console.log(this.index)
        console.log(this.opcode)
        this.opcode === 1 ? { data } = await this.delfunc({ variables: {_id: this.delRegionID}})
        : { data } = await this.addfunc({variables: { region: this.region, _id: this.region.parentregion, index: this.index}})
...
export class EditLandmark_Transaction extends jsTPS_Transaction{
    constructor(regionID, newLandmark, prevLandmark, index, callback){
        super();
        this.regionID = regionID;
    ...
    async doTransaction() {
        const { data } = await this.callback({variables: {_id: this.regionID, landmark: this.newLandmark, index: this.index}})
        return data;
    }

    async undoTransaction() {
        const { data } = await this.callback({variables: {_id: this.regionID, landmark: this.prevLandmark, index: this.index}})
        return data;
    }
}

export class UpdateLandmarkList_Transaction extends jsTPS_Transaction {
    constructor(regionID, landmark, addfunc, delfunc, index, opcode){
        super();
        this.regionID = regionID;
    ...
    async doTransaction(){
        let data;
        this.opcode === 0 ? { data } = await this.delfunc({variables: {_id: this.regionID, index: this.index}})
        : { data } = await this.addfunc({variables: {_id: this.regionID, landmark: this.landmark, index: this.index}})
    ...
    async undoTransaction(){
        let data;
        this.opcode === 1 ? { data } = await this.delfunc({variables: {_id: this.regionID, index: this.index}})
        : { data } = await this.addfunc({variables: {_id: this.regionID, landmark: this.landmark, index: this.index}})
...
export class ChangeParent_Transaction extends jsTPS_Transaction{
    constructor(name, prevname, regionID, currentParent ,callback, user){
        super();
        this.name = name;
    ...
    async doTransaction(){
        const { data } = await this.callback({variables: {name: this.name, _id: this.regionID, currentParent: this.currentParent, user: this.user}})
        this.newparentID = data.changeParent;
        console.log(this.newparentID)
        if(this.newparentID === "error1"){
            alert("There are multiple regions with the same name as the desired parent.")
        }
        else if(this.newparentID === "error2"){
            alert("The desired parent region does not exist.")
        }
        return data;
    }

    async undoTransaction(){
        if(this.newparentID !== "error1" && this.newparentID !== "error2"){
            const { data } = await this.callback({variables: {name: this.prevname, _id: this.regionID, currentParent: this.newparentID, user: this.user}})
            return data;
        }
        return null;
    }
